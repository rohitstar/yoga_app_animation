export default [
    {
      id: '0fa08fa5-6d45-4d56-86e0-8fd56632dfd4',
      role: 'Vrikshasana',
      type: 'Vrikshasana',
      from: '10:52 AM',
      to: '9:40 PM',
      duration: 2,
      distance: 19.7,
      locationImage: require('./assets/images/yoga1.jpg')
    },
    {
      id: 'f8f798bb-c7bd-4080-9bc6-aa4cabfab482',
      role: 'Bhujangasana',
      type: 'Bhujangasana',
      from: '11:45 AM',
      to: '4:08 PM',
      duration: 0,
      distance: 55.2,
      locationImage: require('./assets/images/yoga3.jpg')
    },
    {
      id: '068de8c4-1b87-4051-9885-ee28a48cd274',
      role: 'Sukhasana',
      type: 'Sukhasana',
      from: '9:04 AM',
      to: '9:17 PM',
      duration: 5,
      distance: 1.9,
      locationImage: require('./assets/images/yoga4.jpg')
    },
    {
      id: '269b6feb-7768-41d2-b030-2be94082a974',
      role: 'Trikonasana',
      type: 'Trikonasana',
      from: '11:23 AM',
      to: '7:58 PM',
      duration: 5,
      distance: 38.4,
      locationImage: require('./assets/images/yoga5.jpg')
    },
    {
      id: 'c6ba0009-6a6f-45b0-98cd-730c9c0fd9b0',
      role: 'Adho mukha svanasana',
      type: 'Adho mukha svanasana',
      from: '11:25 AM',
      to: '8:11 PM',
      duration: 3,
      distance: 33.4,
      locationImage: require('./assets/images/yoga1.jpg')
    },
    {
      id: 'ab915f69-ce68-411b-abee-1f5d984af4e2',
      role: 'Power yoga',
      type: 'Power yoga',
      from: '10:22 AM',
      to: '5:37 PM',
      duration: 4,
      distance: 46.4,
      locationImage: require('./assets/images/yoga3.jpg')
    },
    {
      id: '34d383d9-0f41-4c5c-b7fc-9d644b56956c',
      role: 'Hatha yoga',
      type: 'Hatha yoga',
      from: '10:50 AM',
      to: '4:10 PM',
      duration: 4,
      distance: 82.2,
      locationImage: require('./assets/images/yoga4.jpg')
    },
    {
      id: '23ccaf32-7a0b-4652-84e2-5941770006ea',
      role: 'Architect',
      type: 'Dragline',
      from: '9:34 AM',
      to: '2:42 PM',
      duration: 3,
      distance: 60.9,
      locationImage: require('./assets/images/yoga5.jpg')
    },
    {
      id: 'a2c6ba5a-5ead-4a8c-8170-7fbcf7cc74e7',
      role: 'Hatha yoga',
      type: 'Hatha yoga',
      from: '9:17 AM',
      to: '6:57 PM',
      duration: 4,
      distance: 41.0,
      locationImage: require('./assets/images/yoga1.jpg')
    },
    {
      id: '0374d4ca-6db9-4f96-b6b0-48d5365640c6',
      role: 'Electrician',
      type: 'Excavator',
      from: '11:37 AM',
      to: '8:09 PM',
      duration: 1,
      distance: 87.5,
      locationImage: require('./assets/images/yoga3.jpg')
    },
    {
      id: '9c01c773-47b9-47a3-af6d-63f7050d334b',
      role: 'Supervisor',
      type: 'Dump Truck',
      from: '1:00 PM',
      to: '8:57 PM',
      duration: 4,
      distance: 61.0,
      locationImage: require('./assets/images/yoga4.jpg')
    },
    {
      id: 'b48e2d3f-9036-4b30-bde4-11822bcf3b7d',
      role: 'Electrician',
      type: 'Skid-Steer',
      from: '12:31 PM',
      to: '3:31 PM',
      duration: 0,
      distance: 13.6,
      locationImage: require('./assets/images/yoga5.jpg')
    },
    {
      id: '3ae4a618-0992-42da-bf1e-2bd654966705',
      role: 'Supervisor',
      type: 'Dragline',
      from: '8:49 AM',
      to: '6:33 PM',
      duration: 5,
      distance: 19.5,
      locationImage: require('./assets/images/yoga1.jpg')
    },
    {
      id: '77ffe17d-d8e1-463d-a52b-614a5a86bdc9',
      role: 'Hatha yoga',
      type: 'Hatha yoga',
      from: '11:47 AM',
      to: '2:34 PM',
      duration: 3,
      distance: 73.0,
      locationImage: require('./assets/images/yoga3.jpg')
    },
    {
      id: '65eabab9-ae70-43cd-82e1-defb4ba90661',
      role: 'Construction Expeditor',
      type: 'Skid-Steer',
      from: '8:08 AM',
      to: '6:15 PM',
      duration: 0,
      distance: 97.5,
      locationImage: require('./assets/images/yoga4.jpg')
    },
    {
      id: '583b0a0b-525e-4913-9c1d-a723efc729c1',
      role: 'Engineer',
      type: 'Backhoe',
      from: '12:52 PM',
      to: '8:22 PM',
      duration: 3,
      distance: 97.6,
      locationImage: require('./assets/images/yoga5.jpg')
    },
    {
      id: '04fc95ec-dc61-4cdb-9a10-d7fd7c051e82',
      role: 'Electrician',
      type: 'Dragline',
      from: '8:16 AM',
      to: '2:22 PM',
      duration: 3,
      distance: 29.0,
      locationImage: require('./assets/images/yoga1.jpg')
    },
    {
      id: '3911163d-4f64-436a-96ff-8014e5c9947f',
      role: 'Electrician',
      type: 'Dump Truck',
      from: '10:57 AM',
      to: '4:53 PM',
      duration: 5,
      distance: 67.4,
      locationImage: require('./assets/images/yoga1.jpg')
    },
    {
      id: '4112aae1-eb2d-47fb-8c16-fd4849c31ec1',
      role: 'Estimator',
      type: 'Excavator',
      from: '12:15 PM',
      to: '4:14 PM',
      duration: 4,
      distance: 16.0,
      locationImage: require('./assets/images/yoga1.jpg')
    },
    {
      id: '03f15446-7567-445a-aaca-bc5d9459b93a',
      role: 'Construction Expeditor',
      type: 'Trencher',
      from: '8:16 AM',
      to: '6:00 PM',
      duration: 3,
      distance: 22.9,
      locationImage: require('./assets/images/yoga1.jpg')
    },
  ]
  
  export const locationImage =
    'https://miro.medium.com/v2/resize:fit:1200/1*ybR6fbfwo6XTmWvTjXSOAA.png'